...
def Cross_entropy(training_inputs,W1,W2,answer):
    for k, ans in zip(training_inputs, answer):
        v1 = np.matmul(W1,k)#矩阵乘法，第一层神经网络输入（NUM_2*1矩阵）
        y1 = sigmoid(v1)#第二层神经网络输出
        v2 = np.matmul(W2,y1)#第三层神经网络输入
        ...
        dW2 = np.outer(δ2, y1) * alpha
        W2 = W2 + dW2
        Cross_entropy_data.append(e2)
    return W1,W2

for i in range(10000):
    W1, W2 = SGD_double_neural_network(training_inputs,W1,W2,training_answers)
    W3, W4 = Cross_entropy(training_inputs,W3,W4,training_answers)
...
n = range(1,len(st_data)+1)
#创建图形和坐标轴
plt.figure(figsize=(16,9))
plt.plot(n,Cross_entropy_data,label = 'Cross_entropy',alpha = 0.8)
plt.plot(n,st_data,label = 'common',alpha = 0.5)
